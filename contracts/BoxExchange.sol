...
contract BoxExchange {
    ...
    **/
    function _calculatePrice() private returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        if(price >= lowPrice && price <= highPrice){
            ...
            return  _calculateLowPrice(price,
                                        lowPrice,
                                        buyAmount,
                                        buyAmountLimit,
            ...
            return _calculateHighPrice(price,
                                    highPrice,
    ...
    * @dev Refund for all traders if change of price is bigger than SECURE_RATE
    **/
    function _calculateLowPrice(uint256 price,
                                uint256 lowPrice,
                                uint256 buyAmount,
                                uint256 buyAmountLimit,
                                uint256 sellAmount) private returns (uint256, uint256, uint256, uint256, uint256) {
         // executeAmount is amount of buy orders in lowPrice(initial price * 0.999)
        uint256 executeAmount = (((tokenPool.mul(DECIMAL)).add(sellAmount)).decimalDiv(lowPrice)).sub((ethPool.mul(DECIMAL)));
        ...
        if(executeAmount > buyAmount){
            ...
            if( lowPrice > price){
                ...
                executeAmount = (((tokenPool.mul(DECIMAL)).add(sellAmount)).decimalDiv(lowPrice)).sub(ethPool.mul(DECIMAL));
                ...
                if(executeAmount < buyAmount){
                    ...
                    // update price
                    price = ((tokenPool.mul(DECIMAL)).add(sellAmount)).decimalDiv((ethPool.mul(DECIMAL)).add(buyAmount));
    ...
    * @dev Refund for all traders if change of price is bigger than SECURE_RATE
    **/
    function _calculateHighPrice(uint256 price,
                                uint256 highPrice,
                                uint256 buyAmount,
                                uint256 sellAmount,
                                uint256 sellAmountLimit) private returns (uint256, uint256, uint256, uint256, uint256) {
        //executeAmount is amount of sell orders when the price is higher than highPrice(initial price * 1.001)
        uint256 executeAmount = (((ethPool.mul(DECIMAL)).add(buyAmount)).decimalMul(highPrice)).sub(tokenPool.mul(DECIMAL));
        if(executeAmount > sellAmount){
            ...
            if(highPrice < price){
                ...
                executeAmount = (((ethPool.mul(DECIMAL)).add(buyAmount)).decimalMul(highPrice)).sub(tokenPool.mul(DECIMAL));
    ...
    **/
    function _calculatePool(uint256 _buyAmount, uint256 _sellAmount, uint256 _price) private{
         ...
         ethPool = (decimalethPool.add(_buyAmount.decimalMul(DECIMAL + (FEE_RATE.decimalMul(POOL_RATE)))).sub(_sellAmount.decimalDiv(_price))).div(DECIMAL);
         tokenPool = (decimaltokenPool.add(_sellAmount.decimalMul(DECIMAL + (FEE_RATE.decimalMul(POOL_RATE)))).sub(_buyAmount.decimalMul(_price))).div(DECIMAL);
    }
...
//Private functions
    ...
    **/
    function _executionOrders() private {
        ...
        address payable[] storage buyers0 = Exchange[_nextExecuteBoxNumber].buyers;
        address payable[] storage buyers1 = Exchange[_nextExecuteBoxNumber].buyersLimit;
        address payable[] storage sellers0 = Exchange[_nextExecuteBoxNumber].sellers;
        address payable[] storage sellers1 = Exchange[_nextExecuteBoxNumber].sellersLimit;
        ...
        if(length[4] == 0){
            ...
            Exchange[_nextExecuteBoxNumber].unexecutedCategory = uint8(length[4]);
            isSurplus = true;
            Exchange[_nextExecuteBoxNumber].unexecuted = uint8(length[length[4] - 1]);
        ...
        if(category == 1){
            _payment(unexecuted, length[0], price, refundBuy0, true, buyers0, Exchange[_nextExecuteBoxNumber].buyOrders);
        }else{
            _payment(0, length[0], price, refundBuy0, true, buyers0, Exchange[_nextExecuteBoxNumber].buyOrders);
        }
        
        if(category == 2){
            _payment(unexecuted, length[1], price, refundBuy1, true, buyers1, Exchange[_nextExecuteBoxNumber].buyOrdersLimit);
        }else{
            _payment(0, length[1], price, refundBuy1, true, buyers1, Exchange[_nextExecuteBoxNumber].buyOrdersLimit);
        }

        if(category == 3){
            _payment(unexecuted, length[2], price, refundSell0, false, sellers0, Exchange[_nextExecuteBoxNumber].sellOrders);
        }else{
            _payment(0, length[2], price, refundSell0, false, sellers0, Exchange[_nextExecuteBoxNumber].sellOrders);
        }

        if(category == 4){
             _payment(unexecuted, length[3], price, refundSell1, false, sellers1, Exchange[_nextExecuteBoxNumber].sellOrdersLimit);
        }else{
             _payment(0, length[3], price, refundSell1, false, sellers1, Exchange[_nextExecuteBoxNumber].sellOrdersLimit);
        }
        ...
        if(_end > 0){
                for (uint256 i = _start; i < _end; i++) {
                    ...
                    if(_isBuy){
                        ...
                        if(_refundRate < DECIMAL){
                            _transferETH(orderer, (orders[orderer].sub(refundAmount)).decimalDiv(_price));
                        }
                        if (_refundRate > 0){ //if refundrate > 0, refund baseToken
    ...
    **/
    function _transferETH(address _recipient, uint256 _amount) private{
        (bool success, ) = _recipient.call{value: _amount}(abi.encodeWithSignature(""));
